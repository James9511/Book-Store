package com.varsha.bookstore.navigation

const val HOME_URL = "home"
const val SIGNUP_URL = "signup"
const val LOGIN_URL = "login"
const val ADD_PRODUCTS_URL = "add_products"
const val VIEW_PRODUCTS_URL = "view_products"
const val AUTOBIOGRAPHY_URL = "autobiography"
const val COMIC_URL = "comic"
const val DASHBOARD_URL = "dashboard"
const val MOTIVATION_URL = "motivation"
const val NOVEL_URL = "novel"
const val STUDY_URL = "studying"